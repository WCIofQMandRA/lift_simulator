...
{
	m.called_up_floor|=(uint64_t)1<<floor;
	m.waiting_floor=-1;
...
{
	m.called_down_floor|=(uint64_t)1<<floor;
	m.waiting_floor=-1;
	variable::event_queue.push<event_check_lift_state>(time,this);
}
...
{
	m.pressed_button|=(uint64_t)1<<floor;
	m.waiting_floor=-1;
	variable::event_queue.push<event_check_lift_state>(time,this);
}
...
{
	m.called_up_floor&=~((uint64_t)1<<m.floor);
}
void lift_t::remove_called_down_floor()
{
	m.called_down_floor&=~((uint64_t)1<<m.floor);
}
...
{
	m.pressed_button&=~((uint64_t)1<<m.floor);
}
bool lift_t::is_pressed()
{
	return m.pressed_button&((uint64_t)1<<m.floor);
}
bool lift_t::is_pressed_lower()
{
	//避免出现左移64位（此时实际左移了64mod64=0位）
	return m.floor!=0&&m.pressed_button<<(64-m.floor)!=0;
}
bool lift_t::is_pressed_upper()
{
	return m.pressed_button>>(m.floor+1)!=0;
}
bool lift_t::is_called_up()
{
	return m.called_up_floor&((uint64_t)1<<m.floor);
}
bool lift_t::is_called_up_lower()
{
	return m.floor!=0&&m.called_up_floor<<(64-m.floor)!=0;
}
bool lift_t::is_called_up_upper()
{
	return m.called_up_floor>>(m.floor+1)!=0;
}
bool lift_t::is_called_down()
{
	return m.called_down_floor&((uint64_t)1<<m.floor);
}
bool lift_t::is_called_down_lower()
{
	return m.floor!=0&&m.called_down_floor<<(64-m.floor)!=0;
}
bool lift_t::is_called_down_upper()
{
	return m.called_down_floor>>(m.floor+1)!=0;
}
...
uint64_t lift_t::move_to_time(int16_t floor)
{
	...
	if(m.called_down_floor==0&&m.called_up_floor==0&&m.pressed_button==0)
	{
		return m.floor>=floor?(m.floor-floor)*lift_down_tick+lift_down_first_extra_tick+lift_down_last_extra_tick:
		(floor-m.floor)*lift_up_tick+lift_up_first_extra_tick+lift_up_last_extra_tick;
	}
	else if(m.direction<0)
	{
		if(floor<m.floor)
			return (m.floor-floor)*lift_down_tick+lift_down_first_extra_tick+lift_down_last_extra_tick;
		else
		{
			int16_t f=0;
			auto tmp=m.called_up_floor|m.called_down_floor|m.pressed_button;
			while(!(tmp&((uint64_t)1<<f)))++f;
			return 
			//下到f的时间
			((m.floor-f)*lift_down_tick+lift_down_first_extra_tick+lift_down_last_extra_tick)+
			//人进出电梯的时间
	...
	{
		if(floor>m.floor)
			return (floor-m.floor)*lift_up_tick+lift_up_first_extra_tick+lift_up_last_extra_tick;
		else
		{
			...
			return 
			//上到f的时间
			((f-m.floor)*lift_up_tick+lift_up_first_extra_tick+lift_up_last_extra_tick)+
			//人进出电梯的时间
			(ocdoor_tick*2+iolift_tick_range.second)+
			//从f下去的时间
...
{
	return m.up_pressed&((uint64_t)1<<floor);
}

bool wbutton_t::is_down_pressed(int16_t floor)const
{
	return m.down_pressed&((uint64_t)1<<floor);
}
...
void wbutton_t::press_down(uint64_t time,int16_t floor)
{
	...
	{
		using variable::lifts;
		m.down_pressed|=(uint64_t)1<<floor;
		auto t0=lifts[0].move_to_time(floor),t1=lifts[1].move_to_time(floor);
		if(t0<t1||(t0==t1&&rand_between(0,1)))
...
{
	//不在正在开/关门、加/减速时检查
	if(lift->m.is_door_open==1||lift->m.direction==2||lift->m.direction==-2)return;
	using variable::event_queue;
	//正在全速下降
	if(lift->m.direction==-3)
	{
		//需要在当前楼层开门——减速
		if((lift->is_called_down()&&lift->m.carrying_weight<constant::full_weight)
			||lift->is_pressed())
		{
			lift->m.waiting_floor=-1;
			output_time(os<<"[",time)<<"]\t电梯 #"<<lift->m.liftID<<"减速\n\n";
			lift->m.direction=-2;
		...
		{
			lift->m.waiting_floor=-1;
			event_queue.push<event_arrive_at>(time+constant::lift_down_tick,lift,lift->m.floor-1);
		}
		//本层有人按↑
		...
		{
			//呼叫电梯的函数已经把m.waiting_floor设置为-1，所以这个assertion一定失败
			//assert(lift->m.waiting_floor!=-1);
			lift->m.waiting_floor=-1;
		...
		//没有人呼叫电梯并且电梯正在返回待命层
		else if(lift->m.waiting_floor!=-1)
		{
			assert(constant::waiting_floor[lift->m.waiting_floor]<=lift->m.floor);
			if(constant::waiting_floor[lift->m.waiting_floor]==lift->m.floor)
			{
				output_time(os<<"[",time)<<"]\t电梯 #"<<lift->m.liftID<<"到达待命楼层.\n\n";
				lift->m.direction=-2;
	...
	else if(lift->m.direction==3)
	{
		//需要在当前楼层开门——减速
		if((lift->is_called_up()&&lift->m.carrying_weight<constant::full_weight)
			||lift->is_pressed())
		{
			lift->m.waiting_floor=-1;
			output_time(os<<"[",time)<<"]\t电梯 #"<<lift->m.liftID<<"减速\n\n";
		...
		{
			lift->m.waiting_floor=-1;
			event_queue.push<event_arrive_at>(time+constant::lift_up_tick,lift,lift->m.floor+1);
		}
		//本层有人按↓
	...
	else if(lift->m.direction==-1)
	{
		assert(lift->m.waiting_floor==-1);
		if(lift->m.is_door_open==2)
		{
			if(lift->m.passengers[lift->m.floor].size())
				event_queue.push<event_passenger_out>(time+rand_between(constant::iolift_tick_range),lift);
			else if(variable::waiting_queues_down[lift->m.floor].size())
			{
				auto &pass=variable::waiting_queues_down[lift->m.floor].front();
				//乘客已等待超时
				if(pass.appear_time+pass.tolerance_time<time)
				{
					...
					variable::waiting_queues_down[lift->m.floor].pop();
					event_queue.push<event_check_lift_state>(time,lift);
				}
				//电梯没有满员
				else if(lift->m.carrying_weight<constant::full_weight)
					event_queue.push<event_passenger_in>(time+rand_between(constant::iolift_tick_range),lift,-1);
				else
				{
					//电梯满员：电梯关门、乘客重新呼叫电梯
					lift->m.is_door_open=1;
					event_queue.push<event_open_door>(time+constant::ocdoor_tick,lift,false);
					event_queue.push<event_press_wbutton>(time+constant::press_button_tick,-1,lift->m.floor);
				}
			}
			//没有人需要进出
			//在电梯内有人时会按关门键，所以直接关门，否则等待auto_close_door_tick再检查电梯状态
			else if(lift->m.carrying_weight>1e-5
				||lift->m.begin_no_passenger_time+constant::auto_close_door_tick<=time)
			{
				lift->m.is_door_open=1;
				event_queue.push<event_open_door>(time+constant::ocdoor_tick,lift,false);
			}
		...
		//需要在当前楼层开门
			...
			lift->m.is_door_open==0&&(called_down||pressed))
		{
			//如果电梯因呼叫而减速到-1，则其不可能满员
			assert(pressed||lift->m.carrying_weight<constant::full_weight);
			//熄灭按钮
			variable::wall_buttons.switch_off_down(lift->m.floor);
		...
		//需要继续下降，并在底下的楼层开门
			...
			lift->is_pressed_lower())
		{
			output_time(os<<"[",time)<<"]\t电梯 #"<<lift->m.liftID<<"加速\n\n";
			lift->m.direction=-2;
			event_queue.push<event_change_direction>(time+constant::lift_down_first_extra_tick,lift,-3);
		}
		//之前漏了这种情况，导致m.direction被误设为0
		else if(lift->is_called_up())
		{
			variable::wall_buttons.switch_off_up(lift->m.floor);
			lift->m.direction=1;
			lift->m.is_door_open=1;
			event_queue.push<event_open_door>(time+constant::ocdoor_tick,lift,true);
		}
	...
	else if(lift->m.direction==1)
	{
		...
		if(lift->m.is_door_open==2)
		{
			if(lift->m.passengers[lift->m.floor].size())
				event_queue.push<event_passenger_out>(time+rand_between(constant::iolift_tick_range),lift);
			else if(variable::waiting_queues_up[lift->m.floor].size())
			{
				auto &pass=variable::waiting_queues_up[lift->m.floor].front();
				//乘客已等待超时
				if(pass.appear_time+pass.tolerance_time<time)
				{
					event_queue.push<event_passenger_walk>(pass.appear_time+pass.tolerance_time,pass);
					variable::waiting_queues_up[lift->m.floor].pop();
					event_queue.push<event_check_lift_state>(time,lift);
				}
				//电梯没有满员
				else if(lift->m.carrying_weight<constant::full_weight)
		...
		//需要在当前楼层开门
			...
			lift->m.is_door_open==0&&(called_up||pressed))
		{
	...
	{
		assert(lift->m.direction==0);
		assert(lift->m.pressed_button==0);
		assert(fabs(lift->m.carrying_weight)<1e-6);
		//本层楼有人呼叫电梯
		...
		{
			if(lift->m.begin_static_time+constant::return_waiting_floor_tick<=time)
			{
				auto another_lift=&variable::lifts[1-lift->m.liftID];
				switch (another_lift->m.waiting_floor)
				{
				case 0:lift->m.waiting_floor=1;break;
				case 1:lift->m.waiting_floor=0;break;
				default:
				{
					auto d0=std::abs(lift->m.floor-constant::waiting_floor[0]);
					auto d1=std::abs(lift->m.floor-constant::waiting_floor[1]);
					//优先在waiting_floor[0]层等待
					lift->m.waiting_floor=d0<=d1?0:1;
				}
				}
				if(constant::waiting_floor[lift->m.waiting_floor]<lift->m.floor)
				{
					output_time(os<<"[",time)<<"]\t电梯 #"<<lift->m.liftID<<"前往待命楼层.\n\n";
					lift->m.direction=-2;
					event_queue.push<event_change_direction>(time+constant::lift_down_first_extra_tick,lift,-3);
				}
				else if(constant::waiting_floor[lift->m.waiting_floor]>lift->m.floor)
				{
					output_time(os<<"[",time)<<"]\t电梯 #"<<lift->m.liftID<<"前往待命楼层.\n\n";
					lift->m.direction=2;
					event_queue.push<event_change_direction>(time+constant::lift_up_first_extra_tick,lift,3);
			...
			else event_queue.push<event_check_timeout>(lift->m.begin_static_time+constant::return_waiting_floor_tick,lift);
		}
	}
}

event_arrive_at::event_arrive_at(uint64_t time,lift_t *which_lift,int16_t floor):
	event_t(time,"电梯 #"+std::to_string(which_lift->m.liftID)+"到达 "+constant::floor_name[floor]+" 层",
	std::to_string(which_lift->m.liftID)+"arrive_at"+std::to_string(floor)),
...
event_change_direction::event_change_direction(uint64_t time,lift_t *which_lift,int16_t dire):
	event_t(time,"",std::to_string(which_lift->m.liftID)+"change_dire"+std::to_string(dire)),
	lift(which_lift),dire(dire){}

void event_change_direction::call(std::ostream&)const
{
	lift->m.direction=dire;
	variable::event_queue.push<event_check_lift_state>(time,lift);
}	

event_open_door::event_open_door(uint64_t time,lift_t *which_lift,bool open):
	event_t(time,"电梯 #"+std::to_string(which_lift->m.liftID)+(open?"开门":"关门"),
	std::to_string(which_lift->m.liftID)+(open?"open_door":"close_door")),lift(which_lift),open(open){}

void event_open_door::call(std::ostream&)const
{
	lift->m.is_door_open=open?2:0;
	lift->m.begin_no_passenger_time=time;
	variable::event_queue.push<event_check_lift_state>(time,lift);
}

event_passenger_out::event_passenger_out(uint64_t time,lift_t *which_lift):
	event_t(time,"乘客出电梯 #"+std::to_string(which_lift->m.liftID),
	"passengerio"+std::to_string(which_lift->m.liftID)),lift(which_lift){}

void event_passenger_out::call(std::ostream &os)const
{
	auto pass=lift->m.passengers[lift->m.floor].front();
	lift->m.passengers[lift->m.floor].pop();
	lift->m.carrying_weight-=pass.weight;
	os<<"乘客信息: #"<<pass.ID<<", "<<constant::floor_name[pass.source]<<"→"
	<<constant::floor_name[pass.destination]<<", "<<pass.weight<<"kg\n";
	pass.arrive_time=time;
	if(lift->m.passengers[lift->m.floor].empty())
		lift->m.begin_no_passenger_time=time;
...
event_passenger_in::event_passenger_in(uint64_t time,lift_t *which_lift,int16_t dire):
	event_t(time,"乘客进电梯 #"+std::to_string(which_lift->m.liftID),
	"passengerio"+std::to_string(which_lift->m.liftID)),lift(which_lift),dire(dire){}
...
void event_passenger_in::call(std::ostream &os)const
{
	try
	{
		if(dire>0)
		{
			...
			<<constant::floor_name[pass.destination]<<", "<<pass.weight<<"kg\n";
			lift->m.carrying_weight+=pass.weight;
			lift->m.passengers[pass.destination].push(pass);
...
{
	if(lift->m.direction==0&&lift->m.begin_static_time+constant::return_waiting_floor_tick<=time)
	{
		//错误的断言
		//assert(lift->m.called_down_floor==0);
		//assert(lift->m.called_up_floor==0);
		assert(lift->m.pressed_button==0);
		assert(fabs(lift->m.carrying_weight)<1e-6);
		variable::event_queue.push<event_check_lift_state>(time,lift);
	}
