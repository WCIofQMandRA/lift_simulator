...
void event_check_lift_state::call()const
{
	...
	if(lift->m_direction==-2)
	{
		...
		if(lift->is_called_down()||lift->is_pressed())
		{
			...
			event_queue.push(std::make_unique
				<event_change_direction>(time+constant::lift_down_last_extra_tick,lift,-1));
			event_queue.push(std::make_unique<event_check_lift_state>(time+constant::lift_down_last_extra_tick,lift));
		}
		//需要继续下降，并在底下的楼层开门
			...
			lift->is_pressed_lower())
		{
			assert(lift->m_waiting_floor==-1);
			event_queue.push(std::make_unique
				<event_arrive_at>(time+constant::lift_down_tick,lift,lift->m_floor-1));
		}
		//本层有人按↑
	...
	else if(lift->m_direction==2)
	{
		...
		if(lift->is_called_up()||lift->is_pressed())
		{
			assert(lift->m_waiting_floor==-1);
			event_queue.push(std::make_unique
				<event_change_direction>(time+constant::lift_up_last_extra_tick,lift,1));
		}
		//需要继续上升，并在顶上的楼层开门
			...
			lift->is_pressed_upper())
		{
			assert(lift->m_waiting_floor==-1);
			event_queue.push(std::make_unique
				<event_arrive_at>(time+constant::lift_up_tick,lift,lift->m_floor+1));
		}
		//本层有人按↓
	...
	else if(lift->m_direction==-1)
	{
		...
		if(lift->is_called_down()||lift->is_pressed())
		{
			...
			if(!lift->m_is_door_open)
				event_queue.push(std::make_unique
					<event_open_door>(time+constant::ocdoor_tick,lift,true));
		}
		//需要继续下降，并在底下的楼层开门
		else if(lift->is_called_down_lower()||lift->is_called_up_lower()||
			lift->is_pressed_lower())
...
{
	lift->m_floor=floor;
	variable::event_queue.push(std::make_unique<event_check_lift_state>(time,lift));
}

event_change_direction::event_change_direction(uint64_t time,lift_t *which_lift,int16_t dire):
	event_t(time,""),lift(which_lift),dire(dire){}
...
event_open_door::event_open_door(uint64_t time,lift_t *which_lift,bool open):
	event_t(time,"电梯 #"+std::to_string(which_lift->m_liftID)+(open?"开门":"关门")),lift(which_lift),open(open){}
